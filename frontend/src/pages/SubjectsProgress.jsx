...
export default function SubjectsProgress() {
  ...
  useEffect(() => {
    progressService
      ...
      .catch((e) => setErr(e?.message || "Failed to load subjects progress"))
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => (r.subjectName || "").toLowerCase().includes(query));
  }, [rows, q]);
  ...
  return (
    <div className="p-6 space-y-4">
      <div>
        <h1 className="text-2xl font-bold">Subjects Progress</h1>
        <p className="text-gray-600 text-sm">Completion by Subject → Difficulty → Section → Challenge</p>
      </div>
              ...
              filtered.map((r) => (
                <tr key={r.subjectId} className="border-t">
                  ...
                  <td className="px-4 py-3 text-sm">
                    <span className={r.isFinished ? "text-green-600 font-semibold" : "text-gray-600"}>
                      {r.isFinished ? "Finished" : "In progress"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm">
                    <Link
                      className="text-blue-600 hover:underline"
                      to={`/progress/subjects/${r.subjectId}`}
                    >
                      View
                    </Link>
                  </td>
